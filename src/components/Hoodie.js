...
function Hoodie() {
  ...
  const [img, setImg] = useState(collection[2].colors && collection[2].colors[0].image)
  const [prize, setprize] = useState(collection[2].size && collection[2].size[0].rupees)
  const handleClick = (e) => {
    let a = e.currentTarget.value;
    // console.log(a);
    setImg(collection[2].colors && collection[2].colors[a - 1].image)
  }

  const ChangeRupees = (e) => {
    let a = e.currentTarget.value;
    // console.log(a);
    // setImg(collection[1].colors && collection[1].colors[a - 1].image)
    setprize(collection[2].size && collection[2].size[a - 1].rupees)
  }
      ...
      <div className="row men">
            ...
            <div className="small_image">
              <div className="size-container">
                <div className="row pic_changers">
                  {collection[2].colors && collection[2].colors.map(style => {
                    return (
        ...
        <div className=" col-sm-6 partion main_content">
          <div className="content_head">
            <h5> <strong>{collection && collection[2].name} <br />{prize}</strong></h5>
          </div>
          <hr />
          <div className="content_body">
            <div className="information">
              ...
              <div className="row size name">
                ...
                <div className=" col-sm-10 size-container">
                  <div className="size-elements">
                    {collection[2].size && collection[2].size.map(sizes => {
                      return (
                        <div className="size-element size" key={sizes.id} style={{ background: "#d5d5d5" }}>
                          <input type="radio" name="size" value={sizes.id} onChange={ChangeRupees}/>
              ...
              <div className="row name">
                ...
                <div className="col-sm-10 size-container">
                  <div className="size-elements">
                    {collection[2].colors && collection[2].colors.map(rang => {
                      return (
