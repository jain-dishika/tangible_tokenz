...
// const StylecButton = styled.label`
...
function MetalSign() { 
  ...
  const [img, setImg] = useState(collection[4].size && collection[4].size[0].image)
  const [prize, setprize] = useState(collection[4].size && collection[4].size[0].rupees)
  const handleClick = (e) => {
    let a = e.currentTarget.value;
    // console.log(a);
    setImg(collection[4].size && collection[4].size[a - 1].image)
    setprize(collection[4].size && collection[4].size[a - 1].rupees)
  }
  ...
  const highlight = click? 'click': 'noclick';
  return (
      ...
      <div className="row men">
            ...
            <div className="small_image">
              <div className="size-container">
                <div className="row pic_changers">
                  {collection[4].size && collection[4].size.map(style => {
                    return (
                      <div className="col-sm-3 picture " key={style.id}>
                        <input type="radio" name="size" value={style.id} onChange={handleClick} />
        ...
        <div className=" col-sm-6 partion main_content">
          <div className="content_head">
            <h5> <strong>{collection && collection[4].name} <br />{prize}</strong></h5>
          </div>
          <hr />
          <div className="content_body">
            <div className="information">
              ...
              <div className="row size name">
                ...
                <div className=" col-sm-3 size-container">
                  <div className="size-elements">
                    {collection[4].size && collection[4].size.map(sizes => {
                      return (
                        <div className={`size-element size ${highlight}`} key={sizes.id} style={{ background: "#d5d5d5" }}>
                          <input type="radio" name="size" value={sizes.id} onChange={merge} />
              ...
              <div className="row quantity name">
                ...
                <div className="col-sm-10 details size-elements">
                  ...
                  <h4 className='counter numbers'>{state}</h4>
                  ...
                  <div className="col-sm-6 modal_content">
                    ...
                    <div className={`row image_icon${showing}`}>
                      {collection[0].icons && collection[0].icons.map(icon => {
                        return (
                          <div key={icon.id} className="mini_icons col-sm-3">
                            ...
                            {collection[4].size && collection[4].size.map(style => {
                              return (
                                <div className="picture pic" key={style.id}>
                                  ...
                                  <label htmlFor={style.size}><img src={style.image} alt="" srcSet="" /></label>
                                </div>
                              )
                            })}
                          </div>
